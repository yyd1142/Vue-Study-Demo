...
const userInfo = {
    token: 'f67a597402d7ff81a9f165b0c9306628'
};
  ...
  return (params, data) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      axios({
        ...
        timeout: 10000,
        headers: { 'MKOTEAM-ACCESS-TOKEN': userInfo && userInfo.token ? userInfo.token : '' },
      }).then((response) => {
...
export function httpGet(path, domain) {
  return (paths) => {
    return axios({
      method: 'get',
      url: `${domain ? domain : apiConfig.baseUrl}${path}${paths ? paths : ''}`,
      timeout: 10000,
      headers: { 'MKOTEAM-ACCESS-TOKEN': userInfo && userInfo.token ? userInfo.token : '' },
    }).then(response => {
      let result = response.data;
